...
HEADERS = {
    "Authorization": WIX_API_KEY,      # lascia esattamente il formato che stai usando ora
...
# Campi CSV attesi (v7 del TUO file). url_prodotto NON richiesto.
CSV_COLUMNS = [
...
def _desc_html(scadenza: str, eta: str, body_txt: str) -> str:
    scad = (scadenza or "").strip() or "-"
    eta_ = (eta or "").strip() or "-"
    ...
    return prefix + (f"<div><p>{body_html}</p></div>" if body_html else "")
...
def _create_product(row: Dict[str,str], base_price: float, html_desc: str) -> Optional[str]:
    name = _truncate((row.get("nome_articolo") or "").strip(), 80)
    ...
    r = requests.post(f"{API_BASE}/products", headers=HEADERS, data=json.dumps(payload), timeout=40)
    if r.status_code >= 400:
...
def _patch_product(product_id: str, row: Dict[str,str], base_price: float, html_desc: str) -> bool:
    ...
    r = requests.patch(f"{API_BASE}/products/{product_id}", headers=HEADERS, data=json.dumps(payload), timeout=40)
    if r.status_code >= 400:
...
def _patch_variant_prices(product_id: str, base_price: float) -> bool:
    ...
    payload = {
        "variants": [
            {"choices": {OPTION_NAME: CHOICE_DEPOSIT}, "priceData": {"price": deposit}},
            {"choices": {OPTION_NAME: CHOICE_UPFRONT}, "priceData": {"price": upfront}}
        ]
    }
    r = requests.patch(f"{API_BASE}/products/{product_id}/variants", headers=HEADERS, data=json.dumps(payload), timeout=40)
    if r.status_code >= 400:
...
def _add_to_collection(product_id: str, coll_id: str) -> bool:
    r = requests.post(f"{API_BASE}/collections/{coll_id}/products/add",
                      headers=HEADERS, data=json.dumps({"productIds":[product_id]}), timeout=30)
    ...
    with open(CSV_PATH, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if not name or not price or not sku:
                print(f"[SKIP] Riga {riga}: nome_articolo/prezzo_eur/sku mancanti.")
                continue

            print(f"[WORK] Riga {riga}: {name}")

            html_desc = _desc_html(row.get("preorder_scadenza"), row.get("eta"), row.get("descrizione"))

            existing = _query_product_by_sku(sku)
            if existing:
                pid = existing.get("id")
                if not _patch_product(pid, row, price, html_desc):
                    print(f"[ERRORE] PATCH fallita per SKU {sku}")
                    continue
            else:
                pid = _create_product(row, price, html_desc)
                if not pid:
                    print(f"[ERRORE] Creazione fallita per SKU {sku}")
                    continue

            if not _patch_variant_prices(pid, price):
                print(f"[WARN] Aggiornamento prezzi varianti fallito per {pid}")

            cat = (row.get("categoria") or "").strip()
            if cat and coll_map.get(cat):
                _add_to_collection(pid, coll_map[cat])
            elif cat:
