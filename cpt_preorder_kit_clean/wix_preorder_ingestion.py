...
def read_rows(csv_path):
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        for i,row in enumerate(reader, start=2):
            # drop helper columns like __BRANDS__
...
def main():
    ...
    for rownum, r in read_rows(args.csv):
        ...
        try: price = float(r.get("prezzo_eur","0").replace(",","."))
        except: print(f"[ERRORE] Riga {rownum}: prezzo non numerico"); continue
            ...
            product["variants"] = [
                {
                    ...
                    "price": dep,
                    "sku": f"{sku}-DEP" if sku else None,
                    "weight": peso
                },
                {
                    "choices":{"PREORDER PAYMENTS OPTIONS":"PAGAMENTO ANTICIPATO"},
                    "price": full,
                    "sku": f"{sku}-FULL" if sku else None,
                    "weight": peso
                }
            ]
        else:
        ...
        if not descr:
            descr = scraped.get("description") or ""
        if peso is None:
            peso = scraped.get("weight_kg")
        if custom:
            product["customTextFields"] = custom
        ...
        # Collections: categoria + Brand: {brand}
